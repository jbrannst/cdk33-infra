...
oc new-app --as=system:serviceaccount:openshift-infra:metrics-deployer     -f https://raw.githubusercontent.com/openshift/openshift-ansible/master/roles/openshift_hosted_templates/files/v1.3/enterprise/metrics-deployer.yaml -p HAWKULAR_METRICS_HOSTNAME=hawkular-metrics-infra.$APP_DOMAIN  -p USE_PERSISTENT_STORAGE=false -p MODE=validate
